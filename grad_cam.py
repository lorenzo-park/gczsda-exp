...
class FeatureExtractor():
    ...
    def __call__(self, x):
        outputs = []
        self.gradients = []
        for _, m in self.model._modules.items():
            for idx, module in m._modules.items():
                if idx != self.blob_name:
                    ...
                    for name, block in enumerate(getattr(m,self.blob_name)):
                        x = block(x)
...
def show_cams(img, mask_dic, class_name):
    for name, mask in mask_dic.items():
        show_cam_on_image(img, mask, class_name)
...
def show_cam_on_image(img, mask, name):
    ...
    plt.imshow(img)
    plt.imshow(np.uint8(255 * cam), alpha=0.7)
    plt.title(name)
    plt.show()
